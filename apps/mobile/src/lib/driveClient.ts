/**
 * Thin Drive REST v3 wrapper — identical in spirit to apps/web/src/lib/driveClient.ts. Every call
 * is a plain fetch() with no DOM APIs, so this is a straight port rather than a redesign. Only
 * ever touches the one folder/file this app creates for itself (drive.file scope).
 */
